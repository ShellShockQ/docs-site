...
`LAUNCH_COMPONENT_GROUPS`: This is a comma-separated list of which z/OS microservice groups are started when Zowe launches. 
  ...
  - `ZSS` will start the ZSS server without the Desktop its Application Framework server. This can be used with Docker so that the only components that are not contained within Docker are running on z/OS. This may also be useful if you want to utilize ZSS core features or plug-ins without needing the Desktop.
...
**Note: If you are using Docker, it is recommended to remove GATEWAY and DESKTOP from LAUNCH_COMPONENT_GROUPS, such as to set `LAUNCH_COMPONENT_GROUPS=ZSS`. This will prevent duplication of servers running both in Docker and on z/OS**
